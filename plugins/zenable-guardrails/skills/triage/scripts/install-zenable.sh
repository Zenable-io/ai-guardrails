...
if command -v curl >/dev/null 2>&1; then
  curl -fsSL "$url" | bash || echo "[zenable] install via curl failed; run 'curl -fsSL ${url} | bash' manually." >&2
elif command -v wget >/dev/null 2>&1; then
  wget -qO- "$url" | bash || echo "[zenable] install via wget failed; run 'wget -qO- ${url} | bash' manually." >&2
else
